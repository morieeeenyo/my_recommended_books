class Api::V1::BooksController < ApplicationController
  ...
  def create 
    return render status: 404, json: {errors: '推薦図書の投稿にはログインが必要です'} unless @user && @token && @client #どれか一つでもなかったらreturn nil。ステータスは入力しないと204になる
  ...
  def search
    return render status: 404, json: {errors: '書籍の検索にはログインが必要です'} unless @user && @token && @client #どれか一つでもなかったらreturn nil。ステータスは入力しないと204になる
